...
Two typed events, carried as ordinary signed envelopes (E1) with a membership content-type:
...
Because they are DAG messages, membership events are **causally ordered (J5), parked/gap-checked (J4/J6), and applied (J7)** exactly like any other message — the fold reads them in causal order. No new transport, no new ordering machinery.

---

## 4. Content-addressed group identity

The group **ID is the founder's `create` event's envelope hash** (G1) — content-addressed, like every other identifier in the protocol (message IDs, DAG nodes, dedup keys). Consequences:
...
- **One node, always** — the DAG holds a single node per message (J1–J7 unchanged); context is a **read-side lens**, not a partition. There are no per-context DAGs.
...
4. **In-group correlation is unavoidable:** members necessarily learn they share a group (one persona per group message — I6 convergence; per-recipient isolation *within* a group is not available, only *across* contexts).
...
- **E1** envelope (the membership event is a typed payload), **G1** hash (the group ID), **C/D** keystore (the context record is a sibling entry; proof-of-possession for added keys as bundles do), **H** fan-out (an `add` and a group message are fan-outs to the members' encryption keys), **J** (events are DAG messages — ordered/parked/applied), **I6/G6** receive (attribution is a read over the same resolution machinery, unchanged).
...
- **Contact-side origin migration** — replacing bucket D's `confidenceTier` with the origin string. A follow-up so merged D stories are not reopened mid-track.
- **Membership-event schema specifics** beyond what each story needs — pinned per-story (as the backlog does elsewhere).
...
- **Depends on bucket J**: membership events are causal-DAG messages (ordered J5, parked/gap-checked J4/J6, applied J7).
- **No change to the receive core** (composes I6/G6): the core still sees only fan-out units + independent per-member receive. This track **consciously extends** I6's "no group API on the wire" guard — the keyring is **derived, self-facing state above** the core, and a "group message" is a **read**, never a group object *inside* the core or a field *on* the wire.
